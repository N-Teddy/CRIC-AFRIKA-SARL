import { require_react_dom } from './chunk-AS46QCJR.js'
import { __commonJS, __toESM, require_react } from './chunk-SBWQ5KBM.js'
...
var require_dist = __commonJS({
  'node_modules/.pnpm/cookie@1.0.2/node_modules/cookie/dist/index.js'(exports) {
    ...
    function serialize2(name, val, options) {
      ...
      if (options.priority) {
        const priority =
          typeof options.priority === 'string' ? options.priority.toLowerCase() : void 0
        switch (priority) {
          case 'low':
            str += '; Priority=Low'
            break
...
function createRouter(init) {
  ...
  function completeNavigation(location2, newState, { flushSync: flushSync3 } = {}) {
    let isActionReload =
      ...
      location2.state?._isRedirect !== true
    let actionData
...
function UNSTABLE_TransitionEnabledRouterProvider({
  ...
  router2.__setPendingRerender = promise =>
    startRevalidation(
...
function RouterProvider({ router: router2, flushSync: reactDomFlushSyncImpl, unstable_onError }) {
  ...
  let setState = React3.useCallback(
    ...
    [router2.window, reactDomFlushSyncImpl, transition, renderDfd, logErrorsAndSetState]
  )
...
function Router({
  ...
  return React3.createElement(
    ...
    React3.createElement(LocationContext.Provider, { children, value: locationContext })
  )
}
...
function unwrapSingleFetchResult(result, routeId) {
  if ('redirect' in result) {
    let { redirect: location2, revalidate, reload, replace: replace2, status } = result.redirect
...
function getRouteComponents(route, routeModule, isSpaMode) {
  ...
  let ErrorBoundary = routeModule.ErrorBoundary
    ...
    : route.id === 'root'
      ? () => React6.createElement(RemixRootDefaultErrorBoundary, { error: useRouteError() })
      : void 0
  ...
  return React8.createElement(
    ...
    keyedLinks.map(({ key, link }) =>
      isPageLinkDescriptor(link)
        ? React8.createElement(PrefetchPageLinks, { key, nonce, ...link })
        : React8.createElement('link', { key, nonce, ...link })
    )
  )
}
...
function PrefetchPageLinksImpl({ page, matches: nextMatches, ...linkProps }) {
  ...
  return React8.createElement(
    ...
    moduleHrefs.map(href2 =>
      React8.createElement('link', { key: href2, rel: 'modulepreload', href: href2, ...linkProps })
    ),
    ...
    meta.flat().map(metaProps => {
      ...
      if ('charSet' in metaProps && metaProps.charSet != null) {
        return typeof metaProps.charSet === 'string'
          ? React8.createElement('meta', { key: 'charSet', charSet: metaProps.charSet })
          : null
      }
...
function RemixRootDefaultErrorBoundary({ error, isOutsideRemixApp }) {
  console.error(error)
  let heyDeveloper = React9.createElement('script', {
    dangerouslySetInnerHTML: {
      __html: `
        console.log(
...
try {
  if (isBrowser) {
    window.__reactRouterVersion = '7.9.4' // @ts-expect-error
  }
} catch (e) {}
...
function useFetcher({ key } = {}) {
  ...
  let unstable_reset = React10.useCallback(
    opts => router2.resetFetcher(fetcherKey, opts),
    [router2, fetcherKey]
  )
        ...
        ? async args => {
            ...
            if (preRenderedData != null) {
              ...
              if (data2 && SingleFetchRedirectSymbol in data2) {
                ...
                invariant22(data2 && route.id in data2, 'Unable to decode prerendered data')
                let result = data2[route.id]
                invariant22('data' in result, 'Unable to process prerendered data')
                return result.data
              }
            }
...
function RSCDefaultRootErrorBoundaryImpl({ error, renderAppShell }) {
  console.error(error)
  let heyDeveloper = import_react.default.createElement('script', {
    dangerouslySetInnerHTML: {
      __html: `
        console.log(
          "💿 Hey developer 👋. You can provide a way better UX than this when your app throws errors. Check out https://reactrouter.com/how-to/error-boundary for more information."
...
function RSCHydratedRouter({
  ...
  React42.useEffect(() => {
    ...
    async function fetchPatches() {
      ...
      try {
        await fetchAndApplyManifestPatches2(paths, createFromReadableStream, fetchImplementation)
      } catch (e) {
        console.error('Failed to fetch manifest patches', e)
      }
    }
  ...
  return React42.createElement(
    ...
    React42.createElement(
      ...
      React42.createElement(
        ...
        React42.createElement(UNSTABLE_TransitionEnabledRouterProvider, {
          router: router2,
      ...
      ? async (args, singleFetch) => {
          try {
            let result = await match.clientLoader({
              ...args,
              serverLoader: () => {
                preventInvalidServerHandlerCall2('loader', match.id, match.hasLoader)
                if (isHydrationRequest) {
                  if (hasInitialData) {
                    return initialData
                  }
    ...
    action: match.clientAction
      ? (args, singleFetch) =>
          match.clientAction({
            ...args,
            serverAction: async () => {
              preventInvalidServerHandlerCall2('action', match.id, match.hasLoader)
...
async function fetchAndApplyManifestPatches2(
  paths,
  createFromReadableStream,
  fetchImplementation,
  signal
  ...
  return React52.createElement(
    ...
    React52.createElement(
      ...
      React52.createElement(
        FrameworkContext.Provider,
        { value: frameworkContext },
        React52.createElement(StaticRouterProvider, {
          context,
          router: router2,
          hydrate: false,
          nonce: payload.nonce
...
function HydratedRouter(props) {
  ...
  let [criticalCss, setCriticalCss] = React23.useState(true ? ssrInfo?.context.criticalCss : void 0)
  React23.useEffect(() => {
    if (true) {
